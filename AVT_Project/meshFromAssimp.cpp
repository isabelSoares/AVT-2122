...
bool LoadGLTexturesTUs(const aiScene* scene)  // Create OGL textures objects and maps them to texture units.  
{
	...
	if (numTextures) {
		...
		GLuint* textureIds = new GLuint[numTextures + 14];
		glGenTextures(numTextures + 14, textureIds); /* Texture name generation */
		...
		for (int i = 0; itr != textureIdMap.end(); ++i, ++itr)
		{
			...
			filename = (*itr).first;  // get filename
			printf("%s\n", filename.c_str());
			glActiveTexture(GL_TEXTURE0 + i + 14);
			Texture2D_Loader(textureIds, filename.c_str(), i + 14);  //it already performs glBindTexture(GL_TEXTURE_2D, textureIds[i])
